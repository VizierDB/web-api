...
class LogDuration:
    ...
    def __enter__(self):
        ...
        with open("/usr/local/source/web-api/.vizierdb/logs/timing.log", "a") as f:
            f.write('api, ' + self.funcname + ', ' + str(self.uid) + ', start, 0'  + "\n")
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.end = time.time()
        with open("/usr/local/source/web-api/.vizierdb/logs/timing.log", "a") as f:
            f.write('api, ' + self.funcname + ', ' + str(self.uid) + ', end, 0' + "\n")
            f.write('api, ' + self.funcname + ', ' + str(self.uid) + ', duration, ' + str(self.duration()) + "\n")
        return self
