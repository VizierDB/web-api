...
def get_dataset(dataset_id):
    ...
    curation workflow.
    """
    # Get the data with given identifier. The result is None if no dataset with
...
def get_dataset_annotations(dataset_id):
    ...
    annotations = api.get_dataset_annotations(dataset_id)
...
def update_dataset_annotation(dataset_id):
    ...
    obj = validate_json_request(
        request,
        required=['key'],
        optional=['columnId', 'rowId', 'value']
    )
    # Create update statement and execute. The result is None if no dataset with
    # given identifier exists.
    key = obj['key']
    value = obj['value'] if 'value' in obj else None
    if 'columnId' in obj and 'rowId' in obj:
        upd_stmt = UpdCell(obj['columnId'], obj['rowId'], key, value=value)
    elif 'columnId' in obj:
        upd_stmt = UpdCol(obj['columnId'], key, value=value)
    else:
        upd_stmt = UpdRow(obj['rowId'], key, value=value)
    annotations = api.update_dataset_annotation(dataset_id, upd_stmt)
    if not annotations is None:
        return jsonify(annotations)
    raise ResourceNotFound('unknown dataset: ' + dataset_id)
